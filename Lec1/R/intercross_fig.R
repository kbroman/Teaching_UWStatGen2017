...
#png(file="../Figs/intercross.png", width=1125, height=750, res=108,
#       pointsize=14)
if(bw) {
  pdf(file="../Figs/intercross_bw.pdf", width=9.75, height=6.5, pointsize=12, onefile=TRUE)
...
if(bw) {
  rect(c(300,328),c(480,480),c(310,338),c(385,385),lend=1, ljoin=1, lwd=2)
  rect(c(526,554),c(480,480),c(536,564),c(385,385),lend=1, ljoin=1, angle=45, density=20, lwd=2)
} else {
...
if(bw) {
  rect(300,287,310,192, lend=1, ljoin=1, lwd=2)
  rect(328,287,338,192, lend=1, ljoin=1, density=20, angle=45, lwd=2)
  rect(526,287,536,192, lend=1, ljoin=1, lwd=2)
  rect(554,287,564,192, lend=1, ljoin=1, density=20, angle=45, lwd=2)
} else {
...
for(i in 1:10) {
  if(bw) {
    rect(xloc,0,xloc+10,95,    lend=1, ljoin=1, lwd=2)
    rect(xloc+28,0,xloc+38,95, lend=1, ljoin=1, lwd=2)
  } else {
  ...
  for(j in 2:ncol(f2m)) {
    if(f2m[2,j]==2) {
      if(bw) {
        rect(xloc,f2m[1,j]*mult,xloc+10,f2m[1,j-1]*mult,lend=1, ljoin=1, density=20, angle=45, lwd=2)
      } else {
  ...
  for(j in 2:ncol(f2p)) {
    if(f2p[2,j]==2) {
      if(bw) {
        rect(xloc+28,f2p[1,j]*mult,xloc+38,f2p[1,j-1]*mult, lend=1, ljoin=1, angle=45, density=20, lwd=2)
      } else {
