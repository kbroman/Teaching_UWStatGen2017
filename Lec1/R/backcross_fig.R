...
if(bw) {
  pdf(file="../Figs/backcross_bw.pdf", width=9.75, height=6.5, pointsize=12, onefile=TRUE)
} else {
  pdf(file="../Figs/backcross.pdf", width=9.75, height=6.5, pointsize=12, onefile=TRUE)
}
...
if(bw) {
  ...
  rect(c(526,554),c(480,480),c(536,564),c(385,385), angle=45, density=20, lwd=2)
} else {
...
if(bw) {
  rect(413,287,423,192, lend=1, ljoin=1, lwd=2)
  rect(441,287,451,192, lend=1, ljoin=1, angle=45, density=20, lwd=2)
} else {
...
if(bw) {
  rect(413-176,287,423-176,192, lend=1, ljoin=1, lwd=2)
  rect(441-176,287,451-176,192, lend=1, ljoin=1, lwd=2)
} else {
...
for(i in 1:10) {
  ...
  for(j in 2:ncol(f2p)) {
    if(f2p[2,j]==2) {
      if(bw) {
        rect(xloc+28,f2p[1,j]*mult,xloc+38,f2p[1,j-1]*mult,
             lend=1, ljoin=1, density=20, angle=45, lwd=2)
      } else {
        rect(xloc+28,f2p[1,j]*mult,xloc+38,f2p[1,j-1]*mult,
...
text(38-25,95/2,"BC",cex=2,adj=c(1,0.5))
dev.off()
