...
if(bw) {
  pdf(file="../Figs/mixtures_bw.pdf", width=4.5, height=6, pointsize=12, onefile=TRUE)

  par(fg="black",col="black",col.axis="black",col.lab="black", bty="n",
      bg=bgcolor)#, cex.axis=1.5, cex.lab=1.5)
} else {
  pdf(file="../Figs/mixtures.pdf", width=4.5, height=6, pointsize=12, onefile=TRUE)
...
text(51,z+a[3]-0.28,expression(bold(mu[0])),xpd=TRUE,col=color[1])
text(66,z+a[3]-0.28,expression(bold(mu[1])),xpd=TRUE,col=color[1])
dev.off()
